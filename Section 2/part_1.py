...
class cars:
    ...
    def showinfo_2(self):
        ...
        print('(x,y): ',self.row_,',',self.cow_,end=' ')
...
def do_action( s : Stat , AA):
    ...
    if (tmp_type == 5):
        tmp_s.bord[x_][y_ - 1] = 0

        if (A[1] == 'F'):
             cur_car_is.cow_ =  cur_car_is.cow_ - 1
             cur_car_is.Name_ = 'sina'
    ...
    #! AdD Time
    tmp_s.set_car_in_Bord(cur_car_is)
...
def INT_car_Mian(sta : Stat):
    ...
    sta.set_car_in_Bord(red_car)
    sta.set_car_in_Bord(car2)

    # sta.set_car_in_Bord(car3)
    # sta.set_car_in_Bord(car4)
    # sta.set_car_in_Bord(car5)
    # sta.set_car_in_Bord(car6)
    # sta.set_car_in_Bord(car7)
    # sta.set_car_in_Bord(car8)


    # T ==> Yes
def is_goald(stat_ : Stat):
    
    return (stat_.bord[3][0]==5)
    # print("S")
...
if __name__ == '__main__':
    ...
    # # # ! index_ = 2 باید ماشین قرمز بره عقب
    # for index_ in range(0,len(Test_1)):
    #     print()
    #     Test_1[index_][0].showinfo_2()
    #     print('?: ',Test_1[index_][1])
    #     print()
    #     # # ! نباید خود اس اصلی عوض شود
    #     AA = do_action(s_,Test_1[index_]) 
    #     AA.show_Bord()
    ...
    for index_ in range(0,len(Test_1)):
        ...
        AA = do_action(s_,Test_1[index_]) 
        # AA.show_Bord()
        List_of_min_Stat.append(AA)
   

    while (True):
      
        if(   not len(List_of_min_Stat) ): #MT()
            print(":( no so...")
            break
        else:
            cu = List_of_min_Stat.pop(0)
            cu.show_Bord()
            if(is_goald(cu)):
                print("Win!!!!")
                break
            else:
                fooooo = Make_test_Fringe(cu) 
                for i in fooooo:
                    List_of_min_Stat.append(do_action(cu,i))
                    # List_of_min_Ac.append(i)
