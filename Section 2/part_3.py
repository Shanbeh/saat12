...
class cars:
    ...
    def showinfo_2(self):
        ...
        print('(x,y): ',self.row_,',',self.column__,end=' ')
...
if __name__ == '__main__':
    ...
    #! ابن جا به بعد سرکلاس کد می زنیم
    ...
    while (True):
        if(len(List_of_min_Stat) == 0):
            print(" :( ")
            exit(-3)
        Test_1 = List_of_min_Stat.pop(0)
        for i in Test_1:
            if(is_goald(i)):
                print("Yessss")
                exit(+1)
            else:
                List_of_min_Stat.append(do_action(s_,i))
